...
def test_batched_reward_reads_committed_answers():
    ...
    rewards = asyncio.run(math_answer.miles_reward(None, samples))
    assert rewards == [1.0, 1.0, 0.0, 0.0]
    assert asyncio.run(math_answer.miles_reward(None, samples[0])) == 1.0
...
def test_truncated_sample_only_scores_committed_answers():
    ...
    assert asyncio.run(math_answer.miles_reward(None, [truncated, boxed])) == [0.0, 1.0]
