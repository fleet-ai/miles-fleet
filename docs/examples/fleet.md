...
| `env` | environment variables for every pod. The local backend reads `TASKSET_REF`; the authoritative cyber backend reads `FLEET_AUTHORITATIVE_SELECTION` plus its SHA-256. `TASK_LIMIT` is a cost cap ("0" = all tasks). `RUN_ID` defaults to `name`. |
...
The authoritative cyber backend can instead use a checked
`fleet.authoritative-selection.v1` file baked into the immutable trainer image.
That source is deliberately not called TaskDump v7: it rehydrates public task
content under exact hashes, while Fleet's exact-version route provisions the
